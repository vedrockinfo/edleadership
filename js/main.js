...
document.addEventListener("DOMContentLoaded", function () {
    ...
    window.addEventListener('scroll', function () {
        ...
        if (window.scrollY > 200) {
            // If the vertical scroll position is greater than 200 pixels

            // Add the 'fixed-top' class to the navbar_top element
            document.getElementById('navbar_top').classList.add('fixed-top');

            // Add padding top to the body to prevent content from being hidden behind the fixed navbar
            var navbar_height = document.getElementById('navbar_top').offsetHeight;
            document.body.style.paddingTop = navbar_height + 'px';
        } else {
            // If the vertical scroll position is 200 pixels or less

            // Remove the 'fixed-top' class from the navbar_top element
